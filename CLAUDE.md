...
## 0. 詳細度+2の高精細化 ★実装済み・検証済み（あとは push のみ）

> 状態: **実装完了・動作確認済み**（2026-07-20）。設計方針は `docs/DESIGN_CONTEXT.md` が真実の源。
> Claude Code の残タスクは「対象ファイルを明示 add してコミット→push」（DESIGN_CONTEXT §8 P1）。
> 実装内容: `index.html` に `SPR_HI`/`ATLAS_HI`(Base64) と `sprAt()` を追加し、`draw()` で `S.lod>=2` のとき
> `USE_HI=true` にして高精細アトラスを参照。生成は `tools/conform_hi.py` → `assets/hi/atlas_hi.png`+`manifest_hi.json`。
> before/after で草・土・井戸・木・主人公が新素材に置き換わることを確認済み。既知の調整候補は DESIGN_CONTEXT §6 参照。

**やりたかったこと（記録）**: ズームの「詳細度+2」で表示されるドット絵を、より高精細な画像に置き換える。
